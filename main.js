...
async function getmovies(movieList) {
    ...
    let myReq = await fetch(`https://api.themoviedb.org/3/movie/${movieList}?api_key=eba8b9a7199efdcb0ca1f96879b83c44&fbclid=IwAR32Px4_3ZTHYF-tjdSOdkN82Esd5XSCl7c0ueF0LR8urOnlJBZ4TJJdf_k`);
...
async function trending() {
    ...
    let myReq = await fetch(`https://api.themoviedb.org/3/trending/movie/day?api_key=c73c7153f601e122d35a46d43dbd6eca`);
    let finalReq = await myReq.json();
    playingMovies = finalReq.results;
    displayMovie();
}
...
async function search(searchList) {
    ...
    let searchReq = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=c73c7153f601e122d35a46d43dbd6eca&query=${searchList}`);
...
let searchresults2 = document.getElementById("allMovies").addEventListener("keyup", searchList => { search(searchList.target.value) });
let searchresults = document.getElementById("word").addEventListener("keyup", searchList => { search(searchList.target.value) });
